...
genic_structure=list(c(900,1000))
expression_structure=list(c(900:1000))
openchromatin=list(c(900,1000))
k36me2_structure=list(c(900,1000))
k36me3_structure=list(c(900,1000))
...
randomwalkme<-function(chromlength,maxstepsize,maxsteps)
{
  ...
  # test<-randomwalkme(1000,50,5000)
...
#### Notes:
## Make sure everything is modular. The printing K27 methylation should be a module that only wirtes on one nucleosome

# Here are the order of events:
# The clock starts (clock ++)
# PRC2 starts depositing methylation on first nucleosome after its current position based on its conditions
# Checking for mitosis
# if K27M, the clock stalls for a number of rounds (e.g. clock minus 100) with a probability
# repeating this.
